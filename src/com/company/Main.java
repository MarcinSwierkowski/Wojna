...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        while (gracz1.ileKart < 53 & gracz2.ileKart < 53 ){
            System.out.println(gracz1.ileKart + " " +gracz2.ileKart);
            gracz1.przedstawSie();
            gracz2.przedstawSie();
    ...
    private static int graj(KartaDoGry[] talia, Gracz[] tablicaGraczy, int offset) {
        if (talia[tablicaGraczy[0].tablicaKart[offset]].mocFigury > talia[tablicaGraczy[1].tablicaKart[offset]].mocFigury) {
            int j = 0;
            for (int i = 0; i < offset + 1; i++) {
                tablicaGraczy[0].tablicaKart[tablicaGraczy[0].ileKart + i] = tablicaGraczy[0].tablicaKart[i];
                j++;
            }
            for (int i = 0; i < offset + 1; i++) {
                tablicaGraczy[0].tablicaKart[tablicaGraczy[0].ileKart + j] = tablicaGraczy[1].tablicaKart[i];
                j++;
            }
            tablicaGraczy[0].ileKart += 1 + offset;
            tablicaGraczy[1].ileKart -= 1 + offset;
            tablicaGraczy[0].tablicaKart = przesunTablice(tablicaGraczy[0].tablicaKart, 1 + offset, tablicaGraczy[0].ileKart);
            tablicaGraczy[1].tablicaKart = przesunTablice(tablicaGraczy[1].tablicaKart, 1 + offset, tablicaGraczy[1].ileKart);
            return 0;
        }
        if (talia[tablicaGraczy[0].tablicaKart[offset]].mocFigury < talia[tablicaGraczy[1].tablicaKart[offset]].mocFigury) {
            int j = 0;
            for (int i = 0; i < offset + 1; i++) {
                tablicaGraczy[1].tablicaKart[tablicaGraczy[1].ileKart + i] = tablicaGraczy[1].tablicaKart[i];
                j++;
            }
            for (int i = 0; i < offset + 1; i++) {
                tablicaGraczy[1].tablicaKart[tablicaGraczy[1].ileKart + j] = tablicaGraczy[0].tablicaKart[i];
                j++;
            }
            tablicaGraczy[1].ileKart += 1 + offset;
            tablicaGraczy[0].ileKart -= 1 + offset;
